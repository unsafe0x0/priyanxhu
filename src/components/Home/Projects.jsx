  ...
  return (
    <section className="flex justify-center items-center w-full">
      <div className="flex flex-col justify-start items-start w-[95vw] md:w-[80vw] lg:w-[70vw] xl:w-[60vw] 2xl:w-[50vw] px-2 md:px-0 gap-4">
        <h2 className="text-2xl font-normal text-slate-200 underline underline-offset-8 decoration-wavy decoration-cyan-500">
          Things i worked on
        </h2>
          ...
          {projexts.map((project, index) => (
            ...
            >
              <div className="flex justify-between items-center w-full">
                ...
                <Link
                  href={project.github}
                  className="text-xl font-normal text-slate-200 p-2 bg-zinc-950 rounded-md"
                >
                  <FaGithub />
                </Link>
              </div>
              <p className="text-sm md:text-md font-normal text-slate-300">
                {project.desc}
              </p>
              <ul className="flex flex-wrap justify-start items-center gap-2 w-full">
                {project.tech.map((tech, index) => (
                  <li
                    key={index}
                    className="text-lg font-normal text-slate-200 p-1 rounded-md bg-zinc-950"
                  >
                    {tech}
                  </li>
                ))}
